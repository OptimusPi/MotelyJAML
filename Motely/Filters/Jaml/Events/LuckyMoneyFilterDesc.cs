      ...
      IJamlClauseDesc<LuckyMoneyClause>
{
    ...
    public LuckyMoneyFilter CreateFilter(ref MotelyFilterCreationContext ctx)
    {
        Debug.Assert(
            ...
            "LuckyMoney clause must provide at least one roll index."
        );
        int[] sortedRolls = [.. _clause.Rolls];
        Array.Sort(sortedRolls);
        return new LuckyMoneyFilter(sortedRolls, _clause.Min, (double)_clause.With.Luck);
    }

    public struct LuckyMoneyFilter(int[] sortedRolls, int min, double luck) : IMotelySeedFilter
    {
        ...
        public VectorMask Filter(ref MotelyVectorSearchContext ctx)
        {
            ...
            {
                VectorMask trigger = ctx.GetNextLuckyMoney(ref stream, luck);
