      ...
      IJamlClauseDesc<LuckyMultClause>
{
    ...
    public LuckyMultFilter CreateFilter(ref MotelyFilterCreationContext ctx)
    {
        Debug.Assert(
            ...
            "LuckyMult clause must provide at least one roll index."
        );
        int[] sortedRolls = [.. _clause.Rolls];
        Array.Sort(sortedRolls);
        return new LuckyMultFilter(sortedRolls, _clause.Min, (double)_clause.With.Luck);
    }

    public struct LuckyMultFilter(int[] sortedRolls, int min, double luck) : IMotelySeedFilter
    {
        ...
        public VectorMask Filter(ref MotelyVectorSearchContext ctx)
        {
            ...
            {
                VectorMask trigger = ctx.GetNextLuckyMult(ref stream, luck);
