      ...
      IJamlClauseDesc<GrosMichelExtinctClause>
{
    ...
    public GrosMichelExtinctFilter CreateFilter(ref MotelyFilterCreationContext ctx)
    {
        Debug.Assert(
            ...
            "GrosMichel clause must provide at least one roll index."
        );
        int[] sortedRolls = [.. _clause.Rolls];
        Array.Sort(sortedRolls);
        return new GrosMichelExtinctFilter(sortedRolls, _clause.Min, (double)_clause.With.Luck);
    }

    public struct GrosMichelExtinctFilter(int[] sortedRolls, int min, double luck)
        : IMotelySeedFilter
    {
        ...
        public VectorMask Filter(ref MotelyVectorSearchContext ctx)
        {
            ...
            {
                VectorMask trigger = ctx.GetNextGrosMichelExtinct(ref stream, luck);
