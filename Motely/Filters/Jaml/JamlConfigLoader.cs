...
public static partial class JamlConfigLoader
{
    ...
    /// <summary>Parse filter text; format is sniffed (JSON if leading <c>{</c>, else YAML).</summary>
    public static JamlConfig FromJaml(string content) => From(content, JamlLoadFormat.Auto);
    ...
    public static JamlConfig From(string content, JamlLoadFormat format)
    {
        ...
        if (resolved == JamlLoadFormat.Jaml)
            throw new InvalidOperationException(".jaml terse-line documents are not supported; use .yaml, .yml, or .json.");
        try
        {
            JMap root = resolved switch
            {
                ...
                _ => throw new InvalidOperationException($"Unsupported document format: {resolved}."),
            };
    ...
    /// <summary>Leading <c>{</c> is JSON; otherwise YAML.</summary>
    public static JamlLoadFormat Sniff(string content)
    {
        foreach (char c in content)
        {
            if (char.IsWhiteSpace(c))
                continue;
            return c == '{' ? JamlLoadFormat.Json : JamlLoadFormat.Yaml;
        }
        return JamlLoadFormat.Yaml;
    }
